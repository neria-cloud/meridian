...
const pendingTabLabel = "Meridian"
...
func (tm *TabManager) drawHomeOverlay() {
	...
	message := "Meridian CLI"
	detail := "Home"
	if confirm {
		message = "Quit Meridian?"
...
func (tm *TabManager) buildTabBarString() string {
	...
	if noticeText != "" {
		...
		hint = " Quit Meridian?  Ctrl+C: confirm  Esc: cancel "
	} else if scrollMode {
		sbLen := 0
...
func (tm *TabManager) initCursorTrace() {
	...
	if err != nil {
		if tm.stderr != nil {
			fmt.Fprintf(tm.stderr, "meridian: failed to open Claude cursor trace %q: %v\n", path, err)
		}
		return
	}
	tm.cursorTrace = f
