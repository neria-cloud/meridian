...
func TestBuildTabBarStringUsesCLIBrandAndRightSideVersion(t *testing.T) {
	...
	if !strings.Contains(got, "Meridian CLI") {
		t.Fatalf("expected tab bar to contain Meridian CLI branding, got %q", got)
	}
	if strings.Contains(got, " Meridian ") {
		t.Fatalf("did not expect legacy Meridian branding, got %q", got)
	}
...
func TestRemoveLastTabSignalsCloseForAppRelaunch(t *testing.T) {
	...
	if got := out.String(); strings.Contains(got, "Meridian CLI") || strings.Contains(got, "Home") {
...
func TestHomeCtrlCRequiresSecondPressToQuit(t *testing.T) {
	...
	if got := out.String(); !strings.Contains(got, "Quit Meridian?") {
