...
const docsURL = "https://meridian.neria.cloud/quickstart/cli/getting-started"
	...
	case tea.KeyMsg:
		...
		case phaseVirtualKey:
			...
			if s == "f1" {
				...
				if baseURL != "" {
					openBrowser(baseURL)
					m.notify("opened meridian dashboard", false)
				}
				return m, nil
			}
			var cmd tea.Cmd
...
func (m chooserModel) View() string {
	...
	if updateVersion := strings.TrimSpace(m.cfg.UpdateVersion); updateVersion != "" {
		...
		body.WriteString(hint.Render("meridian " + updateVersion + "  "))
	...
	switch m.phase {
	case phaseBaseURL:
		title = accent.Render("Base URL (Meridian Base URL)")
	...
	case phaseHarness:
		...
		if m.returnToSummary {
			...
			title = accent.Render("Meridian CLI")
		} else {
	...
	case phaseSummary:
		...
		renderSummaryLine(summaryActionQuit, "Exit", "Meridian CLI")
