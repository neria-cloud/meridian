...
func TestChooserViewShowsUpdatePrompt(t *testing.T) {
	...
	for _, want := range []string{"Update available:", "meridian v1.2.3", "press y to update now"} {
...
func TestChooserPopupUsesBifrostCLIHeading(t *testing.T) {
	...
	view := m.View()
	if !strings.Contains(view, "Meridian CLI") {
		t.Fatalf("expected selector popup to use Meridian CLI heading, got %q", view)
	}
...
func TestChooserSummaryUsesActionLabels(t *testing.T) {
	...
	if !strings.Contains(view, "Exit") || !strings.Contains(view, "Meridian CLI") {
		t.Fatalf("expected summary to show Exit Meridian CLI, got %q", view)
	}
...
func TestUpdateConfirmUsesHomeStylePopup(t *testing.T) {
	m := confirmModel{
		prompt:    "Meridian v1.2.3 is available. Update now?",
		idx:       1,
		homeStyle: true,
		width:     100,
		height:    30,
	...
	view := m.View()
	for _, want := range []string{"Meridian CLI", "Home", "Update available", "Meridian v1.2.3 is available"} {
		if !strings.Contains(view, want) {
