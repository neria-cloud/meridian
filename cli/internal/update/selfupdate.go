...
func RunSelfUpdate(currentVersion string) error {
	fmt.Println("Checking latest meridian version...")
	...
	fmt.Printf("Updating meridian from %s to %s...\n", currentVersion, latest)

	binaryName := "meridian"
	if runtime.GOOS == "windows" {
		binaryName = "meridian.exe"
	}
	downloadURL := fmt.Sprintf("%s/meridian-cli/%s/%s/%s/%s", baseURL, latest, runtime.GOOS, runtime.GOARCH, binaryName)
	...
	tmpFile, err := os.CreateTemp("", ".meridian-update-*")
	if err != nil {
	...
	fmt.Printf("Updated meridian from %s to %s. Please restart.\n", currentVersion, latest)
	return nil
}
...
func bifrostCLIManagedBinaryPath(binaryName string) (string, error) {
	...
	return filepath.Join(home, ".meridian", "bin", binaryName), nil
}
...
func isWrapperManagedBinaryPath(path, binaryName string) bool {
	...
	cachePrefix := filepath.Clean(filepath.Join(cacheRoot, "meridian")) + string(os.PathSeparator)
...
func stageUpdateBinary(downloadPath, targetPath string, mode os.FileMode) (string, error) {
	stageFile, err := os.CreateTemp(filepath.Dir(targetPath), ".meridian-stage-*")
	if err != nil {
		return "", err
	}
