...
import (
	...
	"github.com/neria-cloud/meridian/cli/internal/apis"
	"github.com/neria-cloud/meridian/cli/internal/harness"
)
...
func AttachBestEffort(ctx context.Context, stdout, stderr io.Writer, h harness.Harness, baseURL, vk string) {
	...
	if strings.TrimSpace(vk) == "" {
		...
		defer cancel()
		cmd := exec.CommandContext(tCtx, "claude", "mcp", "add", "--transport", "http", "meridian", mcpURL)
		if err := cmd.Run(); err != nil {
			fmt.Fprintf(stderr, "warning: auto MCP add failed. Run: claude mcp add --transport http meridian %s\n", mcpURL)
			return
		}
		fmt.Fprintln(stdout, "MCP: attached Claude MCP server 'meridian'.")
		return
	}
	...
	tCtx, cancel := context.WithTimeout(ctx, 10*time.Second)
	defer cancel()
	cmd := exec.CommandContext(tCtx, "claude", "mcp", "add-json", "meridian", payload)
	if err := cmd.Run(); err != nil {
		fmt.Fprintf(stderr, "warning: auto MCP add failed. Run: claude mcp add-json meridian '<payload>' (with your Authorization header)\n")
		return
	}
	fmt.Fprintln(stdout, "MCP: attached Claude MCP server 'meridian' with virtual key.")
}
